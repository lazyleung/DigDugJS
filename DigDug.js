...
function updateMap() {
	...
	switch(player.direction){
		case "right":
			...
			if(arrayPos%24 > 0 && overlay[arrayPos - 1] & 4 === 4) overlay[arrayPos - 1] = overlay[arrayPos - 1] | 4;
			break;
		case "left":
			overlay[arrayPos] = overlay[arrayPos] | 4;
			//console.log(overlay[arrayPos] & 1);
			if(arrayPos%24 < 23 && overlay[arrayPos + 1] & 4 === 4) {
			console.log("hi!");
			overlay[arrayPos + 1] = overlay[arrayPos + 1] | 1;}
			break;
		case "down":
			overlay[arrayPos] = overlay[arrayPos] | 8;
			//if(arrayPos/24 < 22 && overlay[arrayPos - 24] & 2 === 2) overlay[arrayPos - 1] = overlay[arrayPos - 1] | 2;
			break;
		case "up":
